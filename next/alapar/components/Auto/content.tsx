...
export const HtmlAdd = function HtmlAdd(this: React.Component<{}, Auto> & InstanceType<typeof add>) {
    return (<>
        <div className={'part'}>
            ...
            {this.utility.selectGenerator("Elan Növü:", this.state.typeList, "type", this.state.selected.type, this.callbacks.autoTypeCallback)}
            {this.utility.selectGenerator(`${this.state.markTitle}:`, this.state.markList, "mark", this.state.selected.mark, this.callbacks.autoMarkCallback, { search: true })}
            ...
            {this.utility.selectGenerator("Ban növü:", this.state.banList, "ban", this.state.selected.ban, this.callbacks.commonCallback, {visibility: this.state.banType})}
            {this.utility.inputGenerator("Elanın Adı:", "Daxil Edin", this.callbacks.textAreaCallback, "title", { visibility: this.state.title})}
        ...
        <div className={'part'}>
            ...
            {this.utility.selectGenerator("Yanacaq növü:", this.state.fuelList, "fuel", this.state.selected.fuel, this.callbacks.commonCallback, {visibility: this.state.fuel})}
            {this.utility.selectGenerator("Sürətlər qutusu:", this.state.transmissionBoxList, "transmissionBox", this.state.selected.transmissionBox, this.callbacks.commonCallback, { visibility: this.state.transmissionBox})}
            {this.utility.selectGenerator("Ötürücü:", this.state.transmitterList, "transmitter", this.state.selected.transmitter, this.callbacks.commonCallback, {visibility: this.state.transmitter})}
            {this.utility.numberGenerator("Mühərrikin gücü (a.g):", "Daxil Edin", this.callbacks.numberCallback, "power", this.state.power)}
            {this.utility.selectGenerator("Mühərrikin həcmi (sm³):", this.state.capacityList, "capacity", this.state.selected.capacity, this.callbacks.commonCallback, {visibility: this.state.capacity})}
        ...
        <div className="part">
            ...
            <div className={'radioGroup'} style={{ display: 'block!important', paddingBottom: '5px' }}>
                {this.state.checkboxList.map(w => this.utility.checkBoxGenerator(w.title, this.callbacks.checkboxCallback, w.name, this.state.checkboxes, { makeBlock: true }))}
            </div>
        </div>
        <div className="part">
...
export const HtmlFilter = function HtmlFilter(this: React.Component<{}, Auto> & InstanceType<typeof add>) {
    let extras = () => {
        return (
            <>
                {this.utility.selectGenerator("Yanacaq növü:", this.state.fuelList, "fuel", this.state.selected.fuelList, this.callbacks.multipleCallback, { multiple: true, visibility: this.state.fuel })}
                {this.utility.selectGenerator("Sürətlər qutusu:", this.state.transmissionBoxList, "transmissionBoxList", this.state.selected.transmissionBoxList, this.callbacks.multipleCallback, { multiple: true, visibility:  this.state.transmissionBox })}
                {this.utility.selectGenerator("Ötürücü:", this.state.transmitterList, "transmitter", this.state.selected.transmitter, this.callbacks.commonCallback, {visibility: this.state.transmitter})}
                ...
                {this.utility.selectGenerator("Mühərrikin həcmi (sm³):", this.state.capacityList, "capacityList", this.state.selected.capacityList, this.callbacks.multipleCallback, { multiple: true, visibility: this.state.capacity })}

                {this.utility.rangeİnputGenerator("Mühərrikin gücü (a.g):", "minMotorPower", "maxMotorPower", this.callbacks.numberCallback, this.state.power)}

                <div className={'radioGroup'}>
                    {this.state.checkboxList.map(w => this.utility.checkBoxGenerator(w.title, this.callbacks.checkboxCallback, w.name, this.state.checkboxes))}
                </div>
            </>
    ...
    return (<>
        {this.utility.radioGenerator("Satış Növü:", this.state.sellTypeList, 1, this.callbacks.autoSellTypeCallback, "sellType")}
        {this.utility.selectGenerator("Elan Növü:", this.state.typeList, "type", this.state.selected.type, this.callbacks.autoTypeCallback, {categorySave: true})}
        {this.utility.selectGenerator(`${this.state.markTitle}:`, this.state.markList, "mark", this.state.selected.mark, this.callbacks.autoMarkCallback, { search: true })}
        ...
        {this.utility.selectGenerator("Ban növü:", this.state.banList, "banList", this.state.selected.banList, this.callbacks.multipleCallback, { multiple: true, visibility: this.state.banType })}
        ...
        {this.utility.rangeİnputGenerator("Yürüş (km):", "minMileage", "maxMileage", this.callbacks.numberCallback, this.state.mileage)}
        <div className={'radioGroup'}>
            {this.utility.checkBoxGenerator("Kredit", this.callbacks.checkboxCallback, "kredit", this.state.kredit)}
            {this.utility.checkBoxGenerator("Barter", this.callbacks.checkboxCallback, "barter", this.state.barter)}
        </div>
