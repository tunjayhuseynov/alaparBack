...
{
    public class ElectroCrud : Common<ViewElectroAds, ElectroContext, Form, ElectroAds, ElectroLogs, ElectroContacts, ElectroPhotos, ElectroCategories, ElectroStory>, IElectroCrud
    {
        public override string TempFolder => "images/electro/temporarily";
        public override string MainFolder => "images/electro/personal";

        public override Func<ElectroContext, int?, int, int, IAsyncEnumerable<ViewElectroAds>> FilterQuery => EF.CompileAsyncQuery((ElectroContext db, int? id, int skip, int take) => db.ViewElectroAds.Include(w => w.Images).AsNoTracking().AsNoTracking().Where(w => w.CategoryId == id).OrderBy(w => w.ModifiedDate).Skip(skip));
